(() => {
    ...
    const bindSubmit = form => {
        ...
        // ---------- 1. 判断是否 Ninja Forms ----------
        const isNinjaForm = form.classList.contains('ninja-form') ||
                            form.querySelector('.nf-form-cont') !== null ||
                            form.querySelector('[data-form-id]') !== null; // Ninja Forms 常见标记
    
        // ---------- 2. 提前写入隐藏字段（仅 Ninja Forms） ----------
        if (isNinjaForm && !form.dataset.injected) {
            // 根据全局 STEGA 决定是隐写还是明文
            if (STEGA) {
                injectHidden(fld);
            } else {
                injectVisible(fld);
            }
            form.dataset.injected = 'true'; // 防止重复注入
            outline(form, true, "Ninja Forms 已提前注入隐藏字段");
        }
        ...
        bound.add(form);
        outline(form, true, STEGA
            ? (isNinjaForm ? "已绑定·Ninja Forms 提前隐写+镜像" : "已绑定·提交时隐写+镜像")
            : "已绑定·仅镜像（stega=false）"
        );
    };
